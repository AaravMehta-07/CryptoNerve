    ...
    with st.sidebar:
        ...
        if db_ok:
            try:
                ...
                import pandas as pd
                eng = get_engine()
                cutoff = time_ago(hours=24)
                sig_count  = pd.read_sql("SELECT COUNT(*) as c FROM signals WHERE generated_at >= :c", eng, params={"c": cutoff}).iloc[0]["c"]
                news_count = pd.read_sql("SELECT COUNT(*) as c FROM news_articles WHERE published_at >= :c", eng, params={"c": cutoff}).iloc[0]["c"]
            except Exception:
                pass
