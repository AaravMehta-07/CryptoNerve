...
class FearGreedIndex:
    ...
    def calculate(self):
        try:
            ...
            cutoff_24h = time_ago(hours=24)
            sent_df = pd.read_sql("""
                SELECT AVG(avg_sentiment) as avg_sent, SUM(total_posts) as total_volume
                FROM sentiment_aggregated
                WHERE window_start >= :cutoff AND window_size = '1h'
            """, self.engine, params={"cutoff": cutoff_24h})
            ...
            cutoff_24h_v = time_ago(hours=24)
            price_df = pd.read_sql("""
                SELECT close FROM price_data
                WHERE coin = 'BTC' AND interval = '15m'
                AND timestamp >= :cutoff
                ORDER BY timestamp ASC
            """, self.engine, params={"cutoff": cutoff_24h_v})
            ...
            cutoff_whale = time_ago(hours=24)
            whale_df = pd.read_sql("""
                SELECT net_flow_usd, whale_activity_score FROM onchain_metrics
                WHERE timestamp >= :cutoff ORDER BY timestamp DESC LIMIT 1
            """, self.engine, params={"cutoff": cutoff_whale})
    ...
    def get_history(self, hours=168):
        try:
            ...
            return pd.read_sql(
                ...
                ORDER BY timestamp ASC""",
                self.engine, params={"cutoff": cutoff},
            )
        except Exception as e:
