        ...
        try:
            ...
            if not recent.empty:
                ...
                for label, count, color in [
                    ("🟢 Bullish", r["bullish_count"], "#00D4A8"),
                    ("🔴 Bearish", r["bearish_count"], "#FF4C4C"),
                    ("⚪ Neutral", r["neutral_count"], "#4C9BE8"),
                    ("⚠️ FUD", r["fud_count"], "#FFB830"),
                ]:
                    pct = count / total * 100
                    st.markdown(f"""
                    <div style="margin:4px 0;padding:4px 8px;background:#1A1F2E;border-radius:4px;border-left:2px solid {color};">
                        <span style="color:{color};font-size:0.8rem;">{label}</span>
                        <span style="color:#8B95A1;font-size:0.75rem;float:right;">{int(count)} ({pct:.0f}%)</span>
                    </div>
                    """, unsafe_allow_html=True)
