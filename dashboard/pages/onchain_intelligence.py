...
def render():
    ...
    with col2:
        ...
        try:
            whale_df = pd.read_sql(f"""
                SELECT tx_hash, value_usd, tx_type, timestamp,
            ...
            for _, tx in whale_df.iterrows():
                tx_color = {"exchange_inflow": "#FF4C4C", "exchange_outflow": "#00D4A8",
                            "transfer": "#4C9BE8"}.get(tx["tx_type"], "#8B95A1")
                icon = "🏦→🐋" if tx["tx_type"] == "exchange_outflow" else "🐋→🏦" if tx["tx_type"] == "exchange_inflow" else "🐋→🐋"
                st.markdown(f"""
                <div style="padding:6px 10px;margin:3px 0;background:#1A1F2E;border-radius:6px;border-left:3px solid {tx_color};">
                    <div style="color:{tx_color};font-family:'Space Mono',monospace;font-size:0.75rem;">{icon} ${tx['value_usd']:,.0f}</div>
                    <div style="color:#8B95A1;font-size:0.65rem;">{tx['tx_type']} · {str(tx['timestamp'])[:16]}</div>
                </div>
        ...
        try:
            ...
            if not latest_df.empty:
                ...
                <div class="metric-card">
                    ...
                    <div style="color:#8B95A1;font-size:0.75rem;">Activity Score: {r.get('whale_activity_score', 0):.2f}</div>
                </div>
                """, unsafe_allow_html=True)
        except Exception as e:
            st.error(str(e))
