...
class Backtester:
    ...
    def run_backtest(self, coin, days=30):
        signals_df = pd.read_sql(
            ...
            FROM signals
            WHERE coin = %(coin)s AND generated_at > NOW() - (%(days)s || ' days')::INTERVAL
            ORDER BY generated_at ASC
            """,
            self.engine,
            params={"coin": coin, "days": str(days)},
        )
        ...
        prices_df = pd.read_sql(
            ...
            WHERE coin = %(coin)s AND interval = '15m'
        ...
        # HIGH-08 FIX: crypto trades 24/7/365 — use sqrt(365), not sqrt(252)
        sharpe = (
